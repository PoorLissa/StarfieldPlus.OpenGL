...
namespace my
{
    public class myObj_0000 : myObject
    {
        // Priority
        public static int Priority => 10;
		public static System.Type Type => typeof(myObj_0000);
        ...
        protected override void Show()
        {
            switch (shape)
            {
                // Instanced squares
                case 0:
                    myPrimitive._RectangleInst.setInstanceCoords(x - size/2, y - size/2, size, size);
                ...
                // Instanced triangles
                case 1:
                    myPrimitive._TriangleInst.setInstanceCoords(x, y, size/2, angle);
        ...
        protected override void Process(Window window)
        {
            ...
            // Create all the stars and other objects
            {
                ...
                {
                    list.Add(new myObj_0000_StaticStar());
                }

                // Add comets
                for (int i = 0; i < cometsN; i++)
                {
                    list.Add(new myObj_0000_Comet());
                }

                // Add vague roaming lights
                for (int i = 0; i < cometsN; i++)
                {
                    list.Add(new myObj_0000_VagueLight());
                }

                if (doCreateAllAtOnce)
                {
                    while (list.Count < N)
                        list.Add(new myObj_0000_Star());
                }
            }
    ...
    // Moving Star Class
    class myObj_0000_Star : myObj_0000
    {
        protected override void generateNew()
        {
            base.generateNew();
    ...
    class myObj_0000_StaticStar : myObj_0000
    {
        ...
        private List<myObj_0000_StaticStar> neighbours = new List<myObj_0000_StaticStar>();
        ...
        protected override void Show()
        {
            ...
            switch (shape)
            {
                // Instanced squares
                case 0:
                    myPrimitive._RectangleInst.setInstanceCoords(x - size * 0.5f, y - size * 0.5f, size, size);
                    myPrimitive._RectangleInst.setInstanceColor(R, G, B, A * 0.5f);
                    myPrimitive._RectangleInst.setInstanceAngle(angle);

                    myPrimitive._RectangleInst.setInstanceCoords(x - size * 0.5f, y - size * 0.5f, size, size);
                    myPrimitive._RectangleInst.setInstanceColor(R, G, B, A * 0.5f);
                    myPrimitive._RectangleInst.setInstanceAngle(angle + (float)Math.PI * 0.25f);
                    break;

                // Instanced triangles
                case 1:
                    myPrimitive._TriangleInst.setInstanceCoords(x, y, size, angle);
                    myPrimitive._TriangleInst.setInstanceColor(R, G, B, A * 0.5f);

                    myPrimitive._TriangleInst.setInstanceCoords(x, y, size, angle + (float)Math.PI);
                    myPrimitive._TriangleInst.setInstanceColor(R, G, B, A * 0.5f);
                    break;
                ...
                case 3:
                    ...
                    myPrimitive._PentagonInst.setInstanceCoords(x, y, size, angle + (float)Math.PI);
                    myPrimitive._PentagonInst.setInstanceColor(R, G, B, A * 0.75f);
                    break;
        ...
        private void findNeighbours()
        {
            ...
            for (int i = 0; i != Count; i++)
            {
                if (list[i] is myObj_0000_StaticStar)
                {
                    myObj_0000_StaticStar obj = list[i] as myObj_0000_StaticStar;
    ...
    class myObj_0000_Comet : myObj_0000
    {
    ...
    class myObj_0000_VagueLight : myObj_0000
    {
        protected override void generateNew()
        {
            x = rand.Next(gl_Width);
